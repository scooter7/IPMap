"""Merged multi-campaign HTML template (Colorado + Apollo Small + Apollo Large).

Placeholders (replaced by build_merged_dashboard.py):
  /*__CAMPAIGNS__*/   - { key: {label, targetNoun, points, targets, summary, stats} }
  /*__CAMP_ORDER__*/  - [ {key, label}, ... ]  (tab order)
"""
...
<title>Campaign Clicks &rarr; Target Map</title>
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .camp-bar { background: #0d1117; border-bottom: 1px solid #30363d; padding: 8px 20px; display: flex; gap: 8px; align-items: center; flex-shrink: 0; }
  .camp-bar .lbl { font-size: 11px; color: #6e7681; text-transform: uppercase; letter-spacing: .5px; margin-right: 4px; }
  .camp { background: #161b22; border: 1px solid #30363d; color: #adbac7; border-radius: 20px; padding: 6px 16px; font-size: 13px; font-weight: 600; cursor: pointer; }
  .camp:hover { border-color: #58a6ff; color: #f0f6fc; }
  .camp.active { background: #1f6feb; border-color: #1f6feb; color: #fff; }
...
<body>
<header>
  <h1>&#127758; Campaign Clicks &rarr; Target Map</h1>
  ...
  <div class="stat"><span id="s-tnoun">Targets</span> <span id="s-emp"></span></div>
  <div class="stat">Domain-verified <span id="s-domain"></span></div>
  <div class="stat">Matched <span id="s-matched"></span></div>
</header>
<div class="camp-bar"><span class="lbl">Campaign</span><span id="camp-tabs" style="display:flex;gap:8px"></span></div>
<div class="main">
  <div id="map">
    <div class="legend">
      <b>Match basis</b>
      <div><span class="sw" style="background:#a371f7"></span>Domain-verified &mdash; IP hostname = target website</div>
      <div><span class="sw" style="background:#2f81f7"></span>Org-matched &mdash; IP network owner = target</div>
  ...
  <div class="sidebar">
    <div class="tabs">
      ...
      <div class="tab" data-tab="emps" id="tab-emps">By Target</div>
    </div>
    <div class="controls">
      <input id="search" type="text" placeholder="Search IP / target / city..."/>
      ...
      <table id="ip-table">
        <thead id="thead-ips">
          <tr>
            ...
            <th data-col="company" id="th-ip-target">Matched Target</th>
        ...
        <thead id="thead-emps" style="display:none">
          <tr>
            <th data-col="company" id="th-emp-target">Target</th>
...
<script>
const CAMPAIGNS = /*__CAMPAIGNS__*/;
const CAMP_ORDER = /*__CAMP_ORDER__*/;
...
L.control.layers(null, { 'Click locations': markers, 'Targets': empLayer }, { collapsed: false, position: 'topright' }).addTo(map);

// ── per-campaign state ────────────────────────────────────────────────────
let DATA, TARGETS, SUMMARY, STATS, TNOUN, activeKey;
const markerMap = {};
...
function buildLayers() {
  ...
  TARGETS.forEach(e => {
    ...
    }).bindPopup(`<div class="popup-emp">${e.name}</div><div class="popup-detail">${[e.address, e.city, e.state, e.zip].filter(Boolean).join(', ')}</div>${e.website ? `<div class="popup-detail"><a href="${webUrl(e.website)}" target="_blank">${webText(e.website)}</a></div>` : ''}<div class="popup-detail">Targeted ${TNOUN.toLowerCase()} location</div>`)
      .addTo(empLayer);
  });
...
function fitToData() {
  const pts = DATA.filter(d => d.lat != null).map(d => [d.lat, d.lon]);
...
function loadCampaign(key) {
  ...
  activeKey = key;
  DATA = c.points; TARGETS = c.targets; SUMMARY = c.summary; STATS = c.stats; TNOUN = c.targetNoun;
  ...
  document.getElementById('s-domain').textContent  = (STATS.domain || 0).toLocaleString();
  document.getElementById('s-matched').textContent = STATS.matched.toLocaleString();
  document.getElementById('s-tnoun').textContent   = TNOUN + 's';
  document.getElementById('tab-emps').textContent  = 'By ' + TNOUN;
  document.getElementById('th-ip-target').firstChild && (document.getElementById('th-ip-target').textContent = 'Matched ' + TNOUN);
  document.getElementById('th-emp-target').textContent = TNOUN;

  document.querySelectorAll('.camp').forEach(b => b.classList.toggle('active', b.dataset.key === key));

  // reset table/filter state
  view = 'ips'; sortCol = 'clicks'; sortAsc = false; activeId = null;
  document.getElementById('search').value = '';
  document.getElementById('mt-filter').value = '';
...
function render() {
  ...
  document.getElementById('result-count').textContent =
    view === 'ips'
      ...
      : `${rows.length} ${TNOUN.toLowerCase()}${rows.length !== 1 ? 's' : ''} · ${rows.filter(r=>r.clicks>0).length} with matched clicks`;
...
document.getElementById('download-btn').addEventListener('click', () => {
  ...
  let headers, lines;
  if (view === 'ips') {
    headers = ['IP','Clicks','Impressions','Matched '+TNOUN,'Website','Street Address','City','State','Country','Zip','Zip4','Match Type','Match Confidence','Match Distance (mi)','Geocode Precision','IP Hostname','IP City','IP Region','IP Org / ISP','IP Latitude','IP Longitude',TNOUN+' Latitude',TNOUN+' Longitude'];
    ...
    headers = [TNOUN,'Website','Street Address','City','State','Country','Zip','Zip4','Matched IPs','Total Clicks','Total Impressions'];
  ...
  a.download = activeKey + '_' + (view === 'ips' ? 'ip_matches' : 'by_' + TNOUN.toLowerCase()) + '.csv';
  a.click();
});

// ── campaign tabs ───────────────────────────────────────────────────────────
const ctabs = document.getElementById('camp-tabs');
CAMP_ORDER.forEach(c => {
  const b = document.createElement('button');
  b.className = 'camp'; b.dataset.key = c.key; b.textContent = c.label;
  b.addEventListener('click', () => loadCampaign(c.key));
  ctabs.appendChild(b);
});

loadCampaign(CAMP_ORDER[0].key);
</script>
</body>
</html>"""
